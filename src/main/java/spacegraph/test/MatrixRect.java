 ...
 */
public class MatrixRect extends RectNode {
    ...
    //TODO replace with primitive map
    private final Map<Integer, Map<Integer, RectNode>> cell = new HashMap();

    protected RectNode content;
    ...
    public MatrixRect() {
        ...
        content = add(new RectNode());
        content.tangible(false);

        xSlider = add(new Slider(sliderCX, sliderMinCX, sliderMaxCX, new DoubleVar(0.1), Slider.SliderType.Horizontal));
        ySlider = add(new Slider(sliderCY, sliderMinCY, sliderMaxCY, new DoubleVar(0.1), Slider.SliderType.Vertical));
        xSlider.span(-0.4, -0.45, 0.4, -0.5);
        ySlider.span(0.45, -0.4, 0.5, 0.4);


        scaleSlider = add(new Slider(sliderVisHeight, new DoubleVar(1), sliderMaxScale, new DoubleVar(0.2), Slider.SliderType.Vertical));
        widthSlider = add(new Slider(sliderVisWidth, new DoubleVar(1), sliderMaxWidth, new DoubleVar(0.2), Slider.SliderType.Horizontal));
        heightSlider = add(new Slider(sliderVisHeight, new DoubleVar(1), sliderMaxHeight, new DoubleVar(0.2), Slider.SliderType.Vertical));
        ...
        getVisCX().addListener(cl);
        getVisCY().addListener(cl);
        getVisWidth().addListener(cl);
        getVisHeight().addListener(cl);
...
//		});
    ...
    public void put(int x, int y, RectNode r) {
        synchronized (cell) {

            Map<Integer, RectNode> row = cell.get(y);
            if (row == null) {
                row = new HashMap<>();
                cell.put(y, row);
            }
    ...
    public RectNode get(int x, int y) {
        try {
            ...
            for (int y = stopY; y >= startY; y--) {
                ...
                if (row != null) {
                    for (int x = startX; x <= stopX; x++) {
                        ...
                        RectNode r = row.get(x);
                        if (r != null) {
                            r.move(px + xScale / 2.0, py + yScale / 2.0, 0);
...
//			System.out.println(sliderMaxWidth.d() + " , " + sliderMaxHeight.d());
    ...
    public Vector3f getAbsoluteSize() {
        return getWorldScale(); //TODO may not be right
    }
    ...
    public static void main(String[] args) {
        ...
        MatrixRect m = new MatrixRect(/*new DoubleVar(fw), new DoubleVar(fh)*/);

        for (int i = 0; i < w; i++) {
            for (int j = 0; j < h; j++) {
                //RectNode r = new RectNode(Color.newRandom(1.0));
