...
public class JmeWindowLoop extends BaseSimpleGame {
    ...
    public enum ConfigShowMode {
        ...
        AlwaysShow,
        /**
         * Displays a <code>PropertiesDialog</code> only if the properties file
         * is not found or could not be loaded.
         */
        ShowIfNoConfig;
    }
...
//					jme.getVideoState().getSubsamples().i() );
            ...
            whenTitleChanges = getJme().add(new IfStringChanges(getJme().getVideoState().getWindowTitle()) {
                @Override
                public void afterTextChanged(StringVar t, String previous, String current) {
                    updateTitle();
                }
            });
            ...
            whenPixelSizeChanges = getJme().add(new IfVector2Changes(pixelSize) {
                @Override
                public void afterVectorChanged(Vector2 v, double dx, double dy) {
                    recreateWindow();
                }
            });
            whenFullscreenChanges = getJme().add(new IfBoolChanges(jme.getVideoState().getFullScreen()) {
                @Override
                public void afterValueChanged(BooleanVar b, boolean nextValue) {
                    recreateWindow();
                }
            });
